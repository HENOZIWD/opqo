...
import ProgressBar from '@/app/(main)/uploadVideo/modules/components/progressBar/component';
...
import { videoUploaderStyle } from './style.css';
  ...
  return (
    <div className={videoUploaderStyle.container}>
      ...
      {!isUploading
        ? (
          <>
            <input
              type="file"
              id="videoSelector"
              className={videoUploaderStyle.input}
              accept=".mp4,.webm"
              onChange={handleSelectVideo}
            />
            <label
              htmlFor="videoSelector"
              className={videoUploaderStyle.label}
            >
              업로드 할 동영상 파일 선택
            </label>
          </>
        ...
        : (
          <div className={videoUploaderStyle.progress}>
            {isUploadProcessDone
